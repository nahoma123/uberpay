...
type UserPersistence interface {
	...
	GetCompanyUsers(ctx context.Context, companyID uuid.UUID) ([]model.User, error)
...
func (r userPersistence) GetCompanyUserByID(ctx context.Context, user_id uuid.UUID) (*model.CompanyUser, error) {
	...
	err := conn.Model(&model.User{}).Where(model.CompanyUser{UserID: user_id}).First(&cUser).Error
	if err != nil {
...
func (r userPersistence) UserByID(ctx context.Context, param model.User) (*model.User, error) {
	conn := r.conn.WithContext(ctx)
	user := &model.User{}
	err := conn.Model(&model.User{}).Where(model.User{ID: param.ID}).First(&user).Error
	if err != nil {
		return nil, errors.ErrorUnableToFetch
	}
	return user, nil
...
func (r userPersistence) StoreUser(ctx context.Context, user model.User) (*model.User, error) {
	conn := r.conn.WithContext(ctx)
	//user role is given by company
	//user status is approved by company admin
	user.Status = "Pending"
	user.RoleName = ""
...
func (r userPersistence) GetCompanyUsers(ctx context.Context, companyID uuid.UUID) ([]model.User, error) {
	conn := r.conn.WithContext(ctx)
	users := []model.User{}
	err := conn.Model(&model.User{}).Joins("join company_users on company_users.user_id = users.id and company_users.company_id = ?", companyID).Find(&users).Error
	if err != nil {
		return nil, errors.ErrRecordNotFound
	}
	return users, nil
...
func (r userPersistence) RemoveUser(ctx context.Context, parm model.CompanyUser) error {
	...
	err := conn.Transaction(func(tx *gorm.DB) error {
		...
		//	Where(model.User{ID: parm.UserID}).
		//	Delete(model.User{ID: parm.UserID}).Error
		//if err != nil {
		//	return errors.ErrUnableToDelete
		//}
